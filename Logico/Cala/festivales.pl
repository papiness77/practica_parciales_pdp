...
% --- 1 --- %
esReciente(AnioCreacion):-
    anioActual(AnioActual),
    LimiteAntiguedad is AnioActual - 5,
    between(LimiteAntiguedad, AnioActual, AnioCreacion).
...
% --- 2 --- %
bandaCareta(Banda, Bandas):-
    member(Banda, Bandas),
    estaDeModa(Banda).

esCareta(Festival):-   
    festival(Festival, _, Bandas, _),
    bandaCareta(Banda1, Bandas),
    bandaCareta(Banda2, Bandas),
    Banda1 \= Banda2.

esCareta(Festival):-
    festival(Festival, Entradas, _, _),
    member(Entrada, Entradas),
    not(entradaRazonable(Festival, Entrada)).

esCareta(Festival):-
    festival(Festival, _, Bandas, _),
    member(tanBionica, Bandas).

% --- 3 --- %
valorEntrada(PrecioBase, plateaGeneral(_), Valor):-
